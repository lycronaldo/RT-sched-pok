...
### 3.3 带权重的round-robin调度场景
**场景设置：**浏览器图形界面移动（软实时）

* 鼠标移动线程： 负责与用户交互。该线程的响应速度要尽量快，权重最高。
* 界面渲染线程： 负责图形界面的细节渲染，可以比鼠标移动的线程满一些，但是必须在数据更新线程前执行完毕。
* 数据更新线程： 负责更新用户界面的数据，由于可能涉及I/O的缘故，最慢的执行完成。

三个线程并非完全独立，因为显然不能出现鼠标能够移动但是界面是空白的情况，因此三个线程必须交叉执行。
