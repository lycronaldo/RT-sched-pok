...
void pok_partition_reinit(const uint8_t pid) {
    ...
    pok_partitions[pid].prev_thread =
        IDLE_THREAD; // breaks the rule of prev_thread not being idle, but it's just for init
...
#include <libc.h>
        ...
        pok_partitions[i].thread_index_high =
            pok_partitions[i].thread_index_low + ((uint32_t[])POK_CONFIG_PARTITIONS_NTHREADS)[i];
        ...
        for (i = 0; i < pok_partitions[pid].nthreads; i++) {
            ...
            if ((long long)thread->period == -1) { //-1 <==> ARINC INFINITE_TIME_VALUE
                if (thread->state == POK_STATE_DELAYED_START) { // delayed start, the delay is in the wakeup time
                    ...
                    if (thread->time_capacity > 0) thread->end_time = thread->wakeup_time + thread->time_capacity;
                }
            } else {
                if (thread->state == POK_STATE_DELAYED_START) { // delayed start, the delay is in the wakeup time
                    if (!thread->wakeup_time) {
