...
#endif
        if ((POK_SCHED_CURRENT_THREAD != IDLE_THREAD) && (POK_SCHED_CURRENT_THREAD != POK_CURRENT_PARTITION.thread_main)
#ifdef POK_NEEDS_ERROR_HANDLING
...
void pok_sched_thread_switch() {
    ...
    for (i = 0; i <= POK_CONFIG_NB_THREADS; ++i) {
        if ((pok_threads[i].state == POK_STATE_WAITING) && (pok_threads[i].wakeup_time <= now)) {
...
uint32_t pok_sched_part_rms(const uint32_t index_low, const uint32_t index_high,
    ...
    } while ((res != index_low) && (pok_threads[res].state != POK_STATE_RUNNABLE));
...
uint32_t pok_sched_get_current(uint32_t* thread_id) {
    ...
    if (pok_partitions[pok_current_partition].thread_error == 0) return POK_ERRNO_THREAD;
#endif
